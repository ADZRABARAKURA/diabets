...
input_data = [answer_diabets, answer_BP, answer_HighChol, answer_CholCheck, answer_BMI, answer_Smoker, answer_Stroke, answer_HeartDiseaseorAttack,
              ...
              answer_GenHlth, answer_MentHlth, answer_PhysHlth, answer_DiffWalk, answer_Sex, answer_Age, answer_Income]
...
if result:
    if(answer_diabets == 1 or  answer_diabets == 2):
        ...
        st.title("Состояние вашего здоровья вызывает у нас подозрения, у вас возможен диабет, советуем в ближайшее время обратиться к терапевту и рассказать ему о ваших проблемах со здоровьем")
    elif(prediction == 0):
